...
extern "C"
{
	...
	FREObject listInputDevices(FREContext ctx, void* funcData, uint32_t argc, FREObject argv[])
	{
        ...
        char msg[256];
        sprintf(msg,"listInputDevices > %i devices detected :\n",nPorts);
        as3Print(msg);
		...
		for ( unsigned int i=0; i<nPorts; i++ ) {
			...
			FREObject exc = NULL;
			FRENewObject((const uint8_t *)"benkuper.nativeExtensions.MIDIDeviceIn",1,args,&curPort,&exc);
	...
	FREObject listOutputDevices(FREContext ctx, void* funcData, uint32_t argc, FREObject argv[])
	{
		...
		char msg[256];
        sprintf(msg,"listOutputDevices > %i devices detected :\n",nPorts);
        as3Print(msg);
		FRESetArrayLength(result,nPorts);
		...
		for ( unsigned int i=0; i<nPorts; i++ ) {
			...
			FREObject curPort = NULL;
			FREObject exc = NULL;
			FRENewObject((const uint8_t *)"benkuper.nativeExtensions.MIDIDeviceOut",1,args,&curPort,&exc);
	...
	void NativeMIDIContextInitializer(void* extData, const uint8_t* ctxType, FREContext ctx, uint32_t* numFunctionsToSet,  const FRENamedFunction** functionsToSet) { 
		...
		printf("** Native MIDI Extension v0.2 by Ben Kuper **\n");
